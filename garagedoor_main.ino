...
// wifi credentials stored externally and .gitignore
 ...
 uint16_t RELAY_OUT_PIN = 14;
 // GPIO Relay in for status of door
 uint16_t RELAY_IN_PIN = 10;
...
void send_status()
 ...
 void sendSensorValues(){
   ...
   //BME280 Humidity
    ...
    //GARAGE Door status
    strcpy(outTopic_status,outTopic);
    strcat(outTopic_status,"garagedoor_status");
    dtostrf(garage_door_status,2,2,msg);
    client.publish(outTopic_status,msg ); 
 ...
 void update_sensors() {
   ...
   bme280_height=bme280.readAltitude(SEALEVELPRESSURE_HPA); //m
    garage_door_status=digitalRead(RELAY_IN_PIN);
 }
